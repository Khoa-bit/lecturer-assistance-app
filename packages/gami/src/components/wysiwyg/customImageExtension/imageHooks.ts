...
const useImage: useImageType = (pbClient, documentId, setCurAttachments) => {
  const imageProxy: ImageProxyType = useCallback(
    async (imageBlob) => {
    ...
    [documentId, pbClient, setCurAttachments]
  );

  const imageHandleDrop: ImageHandleDropType = useCallback(
    (view, event, moved) => {
      if (
        moved ||
        !event.dataTransfer ||
  ...
  const addImage = useCallback(
    async (editor: Editor, alt?: string, title?: string) => {
